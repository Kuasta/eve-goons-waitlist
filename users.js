...
const waitlist = require("./globalWaitlist");

module.exports = function (setup) {
    var module = {};

    /* This nested if stuff is kinda unpleasant and I'd like to fix it */
    module.updateUserSession = (req, res, next) => {
        if (
        ...
        module.findAndReturnUser(
            req.session.passport.user.characterID,
            function (userData) {
                if (!userData) {
                    req.logout();
                    res.redirect("/");
                    ...
                    req.session.save(function (err) {
                        if (err)
                            log.error(
                                "updateUserSession: Error for session.save",
                                {err, "characterID": userData.characterID}
                            );
                    });
    ...
    /* Create and manage users - Currently doing this via JSON and saving
    the object every now and then. TODO: MongoDB with mongoose maybe? */
    module.findOrCreateUser = function (
        ...
        module.findAndReturnUser(characterDetails.CharacterID, function (userProfile) {
            ...
            if (userProfile) {
                ...
                generateNewUser(
                    refreshToken, characterDetails, null, null,
                    function (userProfile, err) {cb(userProfile, err);}
                );
            }
        });
    };
    ...
    module.findAndReturnUser = function (checkID, cb) {
        db.find({"characterID": checkID}).toArray(function (err, docs) {
            if (err)
                log.error(
                    "findAndReturnUser: Error for db.find.toArray", {err, checkID}
                );
            if (docs.length === 0) {
                cb(false);
            } else {
    ...
    module.updateRefreshToken = function (checkID, token) {
        db.updateOne(
            {"characterID": checkID}, {$set: {refreshToken: token}},
            function (err, result) {
                if (err)
                    log.error(
                        "updateRefreshToken: Error for updateOne",
                        {err, "characterID": checkID}
                    );
            }
        );
    };
    ...
    module.getLocation = function (user, cb, passthrough) {
        module.findAndReturnUser(user.characterID, function (newUser) {
            ...
            refresh.requestNewAccessToken(
                "provider",
                user.refreshToken,
                function (err, accessToken, newRefreshToken) {
                    if (err) {
                        log.error(
                            "getLocation: Error for requestNewAccessToken",
                            {err, characterID: user.characterID}
                        );
                        if (err.data.error.includes("invalid_token")) {
                            log.error(
                                "requestNewAccessToken has failed due to invalid token, removing them from waitlist."
                            );
                        ...
                        esi.characters(user.characterID, accessToken)
                        .location()
                        .then(function (locationResult) {
                            cache.get(
                                [locationResult.solar_system_id],
                            ...
                            log.error(
                                "users.getLocation: Error for esi.characters",
                                {err, characterID: user.characterID}
                            );
                        });
                    }
            ...
            esi.corporations.names(corporationID).then(function (corporation) {
                if (allianceID !== 0) {
                    esi.alliances.names(allianceID).then(function (alliance) {
                        ...
                        log.error(
                            "users.getUserDataFromID: Error for esi.alliances.names",
                            {err, userId: id, allianceID}
                        );
                    });
                } else {
                ...
                log.error(
                    "users.getUserDataFromID: Error for esi.corporations.names",
                    {err, userId: id, corporationID}
                );
            });
        }).catch(err => {
            log.error(
                "users.getUserDataFromID: Error for esi.characters.info",
                {err, id}
            );
        });
    };
    ...
    var generateNewUser = function (
        ...
        characterDetails,
        masterAccount,
        associatedMasterAccount,
        cb
    ) {
        module.getUserDataFromID(
            characterDetails.CharacterID,
            function (alliance, corporation) {
                if (
                    alliance &&
                    setup.permissions.alliances.includes(alliance.name)
                ) {
                    ...
                    db.insert(newUserTemplate, function (err, result) {
                        if (err)
                            log.error(
                                "generateNewUser: Error for db.insert",
                                {err, name: characterDetails.CharacterName}
                            );
                        cb(newUserTemplate);
                    });
                } else {
    ...
    module.updateUserPermission = function (characterID, permission, adminUser, cb) {
        ...
        if (characterID !== adminUser.characterID) {
            db.updateOne(
                ...
                {
                    $set: {
                        roleNumeric: Number(permission),
                        role: setup.userPermissions[permission]
                    }
                },
                function (err, result) {
                    if (err)
                    ...
                    if (!err)
                        log.debug(
                            adminUser.Name + " changed the role of " + characterID + " to " + setup.userPermissions[permission]
                        );
                }
            );
        }
    ...
    module.setDestination = function (user, systemID, cb) {
        refresh.requestNewAccessToken(
            ...
            function (err, accessToken, newRefreshToken) {
                if (err) {
                    log.error(
                        "module.setDestination: Error for requestNewAccessToken",
                        {err, user}
                    );
                    cb(err);
                } else {
                    ...
                    esi.characters(user.characterID, accessToken)
                    .autopilot.destination(systemID)
                    .then(result => {
                        cb("OK");
                    }).catch(err => {
    ...
    module.showInfo = function (user, targetID, cb) {
        refresh.requestNewAccessToken(
            ...
            function (err, accessToken, newRefreshToken) {
                if (err) {
                    log.error(
                        "module.showInfo: Error for requestNewAccessToken",
                        {err, user}
                    );
                    cb(err);
                } else {
